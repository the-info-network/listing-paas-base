-- =============================================================================
-- V1 CONSOLIDATED MIGRATION: STRIPE BILLING
-- =============================================================================
-- This migration creates all Stripe-related tables for billing:
...
-- Users can view their tenant's Stripe data
...
CREATE POLICY "Authenticated users can view products"
  ON stripe_products FOR SELECT USING (true);

CREATE POLICY "Authenticated users can view prices"
  ON stripe_prices FOR SELECT USING (true);

-- =============================================================================
-- COMMENTS
-- =============================================================================
COMMENT ON TABLE stripe_customers IS 'Stripe customer records synced from Stripe';
COMMENT ON TABLE stripe_subscriptions IS 'Active and historical subscription records';
COMMENT ON TABLE stripe_payment_methods IS 'Saved payment methods (cards, bank accounts)';
COMMENT ON TABLE stripe_invoices IS 'Invoice records for billing history';
COMMENT ON TABLE stripe_payment_intents IS 'Payment intent tracking for one-time payments';
COMMENT ON TABLE stripe_webhook_events IS 'Webhook event log for debugging and idempotency';
COMMENT ON TABLE stripe_products IS 'Product catalog synced from Stripe';
COMMENT ON TABLE stripe_prices IS 'Price definitions for products';
