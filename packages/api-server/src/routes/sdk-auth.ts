...
sdkAuthRoutes.post('/auth', async (c) => {
  try {
    ...
    return errors.internalError(c, error instanceof Error ? error.message : 'Failed to create API key');
  }
});
...
sdkAuthRoutes.get('/auth/keys', async (c) => {
  try {
    ...
    return errors.internalError(c, error instanceof Error ? error.message : 'Failed to list API keys');
  }
});
...
sdkAuthRoutes.delete('/auth/keys/:id', async (c) => {
  try {
    ...
    return errors.internalError(c, error instanceof Error ? error.message : 'Failed to revoke API key');
  }
});
...
sdkAuthRoutes.post('/webhooks/booking/subscribe', async (c) => {
  try {
    ...
    return errors.internalError(c, error instanceof Error ? error.message : 'Failed to subscribe to webhooks');
  }
});
...
sdkAuthRoutes.get('/webhooks/booking', async (c) => {
  try {
    ...
    return errors.internalError(c, error instanceof Error ? error.message : 'Failed to list webhook subscriptions');
  }
});
...
sdkAuthRoutes.get('/webhooks/booking/:id', async (c) => {
  try {
    ...
    if (!data) {
      return errors.notFound(c, 'Webhook subscription not found');
    }

    return success(c, { subscription: data });
  } catch (error) {
    return errors.internalError(c, error instanceof Error ? error.message : 'Failed to get webhook subscription');
  }
});
...
sdkAuthRoutes.delete('/webhooks/booking/:id', async (c) => {
  try {
    ...
    return errors.internalError(c, error instanceof Error ? error.message : 'Failed to unsubscribe from webhooks');
  }
});
...
sdkAuthRoutes.get('/webhooks/booking/deliveries', async (c) => {
  try {
    ...
    if (!subscriptionId) {
      return errors.badRequest(c, 'subscriptionId is required');
    }
    ...
    return errors.internalError(c, error instanceof Error ? error.message : 'Failed to list webhook deliveries');
  }
});
